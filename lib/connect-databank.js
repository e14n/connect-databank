        ...
        Step(
            ...
            function(err, sids) {
                if (err && err.name == "NoSuchThingError") {
                    ...
                    if (store.log) store.log.info({sids: sids, length: sids.length}, "Checking sessions.");
                    bank.readAll("session", sids, this);
                }
            },
            function(err, sessionMap) {
                var sid, sessions = [], group = this.group();
                if (err) throw err;
                for (sid in sessionMap) {
                    if (store.log) store.log.debug({sid: sid}, "Checking session for cleanup.");
                    if (sessionMap[sid] && !store.isActive(sessionMap[sid])) {
                        if (store.log) store.log.info({sid: sid, session: sessionMap[sid]}, "Cleaning up old session.");
                        store.destroy(sid, group());
                    }
                }
            },
            function(err) {
