    ...
    var DatabankStore = function(bank, parentLog, cleanupInterval) {
        ...
        store.get = function(sid, callback) {
            ...
            async.waterfall([
                function(callback) {
                    bank.read("session", sid, function(err, session) {
                        if (err && err.name == "NoSuchThingError") {
                            if (log) log.info({sid: sid}, "No session");
                            callback(null, null);
                        } else if (err) {
                            callback(err, null);
                        } else {
                        ...
                        setImmediate(function() {
                            bank.del("session", sid, function(err) {
                                if (err) {
                                    ...
                                    if (log) log.info({sid: sid}, "Inactive session; deleted.");
                                }
                            });
                        });
                        callback(null, null);
                ...
                if (err) {
                    ...
                    if (log) log.info({sid: sid, session: session}, "Got session.");
                    callback(null, session);
                }
            });
        };
        ...
        store.set = function(sid, sess, callback) {
            ...
            async.waterfall([
                ...
                if (err) {
                    ...
                    if (log) log.info({sid: sid, session: sess}, "Saved session.");
                    callback(null);
                }
            });
        };
        ...
        store.destroy = function(sid, callback) {
            ...
            async.waterfall([
                ...
                if (err && err.name == "NoSuchThingError") {
                    if (log) log.info({sid: sid}, "Destroy for non-existent session; all good.");
                    callback(null);
                } else if (err) {
                    if (log) log.error(err);
                    callback(err);
                } else {
                    if (log) log.info({sid: sid}, "Destroy for found session");
                    callback(null);
                }
            });
        };
        ...
        store.all = function(callback) {
            ...
            async.waterfall([
                ...
                if (err) {
                    ...
                    if (log) log.info({count: sessions.length}, "Retrieved all sessions");
                    callback(null, sessions);
                }
            });
        };
        ...
        store.length = function(callback) {
            ...
            async.waterfall([
                ...
                if (err) {
                    ...
                    if (log) log.info({length: lng}, "Retrieved count of sessions");
                    callback(null, lng);
                }
            });
        };
        ...
        store.clear = function(callback) {
            ...
            async.waterfall([
                ...
                if (err) {
                    ...
                    if (log) log.info("Cleared all sessions.");
                    callback(null);
                }
            });
        };
        ...
        store.cleanup = function(callback) {
            ...
            var cid = uuid.v4(),
                ...
                cleanupSession = function(sid, callback) {
                    if (log) log.info({sid: sid, cid: cid}, "Deleting inactive session.");
                    bank.del("session", sid, function(err) {
                        if (err && err.name == "NoSuchThingError") {
                            if (log) log.info({sid: sid, cid: cid}, "Missing inactive session on delete; ignoring.");
                            callback(null);
                        } else if (err) {
                            if (log) log.error({sid: sid, cid: cid, err: err}, "Error deleting this session.");
                            callback(err);
                        } else {
                            if (log) log.info({sid: sid, cid: cid}, "Successfully deleted session.");
                            callback(null);
                        }
                    });
                },
                check = function(session) {
                    setImmediate(function() {
                        if (isActive(session)) {
                            if (log) log.info({sid: session.sid, cid: cid}, "Ignoring active session.");
                        } else {
                            if (log) log.info({sid: session.sid, cid: cid}, "Queuing inactive session for cleanup.");
                            q.push(session.sid); 
                        }
                    });
                },
            ...
            if (log) log.info({cid: cid}, "Starting cleanup.");

            q = async.queue(cleanupSession, 128);

            q.drain = function() {
                if (log) log.info({cid: cid}, "Queue is empty.");
                if (scanDone) {
                    if (log) log.info({cid: cid}, "Finished cleaning up.");
                    callback(null);
                }
            };
            ...
            bank.scan("session", check, function(err) {
                if (err) {
                    ...
                    if (log) log.info({cid: cid}, "Finished scanning sessions; waiting for queue to drain.");
                    scanDone = true;
                }
            });
        };
                ...
                var doCleanup = function() {
                    store.cleanup(function(err) {
                        if (err) {
                            ...
                            if (log) log.info("Finished cleaning up sessions.");
                        }
                    });
                };
                doCleanup();
