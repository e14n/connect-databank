...
module.exports = function(connect) {
    ...
    var DatabankStore = function(bank, parentLog, cleanupInterval) {
        ...
        store.cleanup = function(callback) {
            ...
            if (log) log.info("Starting cleanup.");
            ...
            async.waterfall([
                function(callback) {
                    var toDel = [],
                        check = function(session) {
                            if (isActive(session)) {
                                if (log) log.info({sid: session.sid}, "Ignoring active session.");
                            } else {
                                if (log) log.info({sid: session.sid}, "Queuing inactive session for cleanup.");
                ...
                function(toDel, callback) {
                    ...
                    if (log) log.info({count: toDel.length}, "Deleting inactive sessions.");
                ...
                if (err) {
                    ...
                    if (log) log.info("Cleanup complete");
                    callback(null);
                }
            });
        };
